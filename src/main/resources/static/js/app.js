(() => {
  const statusEl=document.querySelector('#status'),start=document.querySelector('#start'),stop=document.querySelector('#stop');
  ...
  const updateButtons=()=>{start.disabled=busy||['RUNNING','STARTING'].includes(current);stop.disabled=busy||['STOPPED','STOPPING'].includes(current)};
  ...
  request('api/status','GET',true);setInterval(()=>{if(!busy)request('api/status','GET',true)},10000);
})();
